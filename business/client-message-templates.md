# Client Message Templates

## New Inquiry Reply

Hi [NAME]! Thank you for reaching out about Papa Noel.

Can you please send:

- Event date
- Start time
- City/address area
- Event type
- Estimated number of children/adults
- How long you need Santa
- Whether Santa will hand out gifts

Once I have that, I can confirm availability and package options.

## Price Message

For your event, I recommend the [PACKAGE NAME].

It includes:

- [DETAIL 1]
- [DETAIL 2]
- [DETAIL 3]

Total: [$___]  
Deposit to reserve: [$___]  
Remaining balance: [$___]

The date/time is confirmed once the deposit is received.

## Deposit Request

To reserve Papa Noel for [DATE] at [TIME], the deposit is [$___].

Payment options:

- Zelle: [PAYMENT INFO]
- Cash App: [PAYMENT INFO]
- Venmo: [PAYMENT INFO]

Once received, I will send the confirmation details.

## Confirmation

Confirmed! Papa Noel is booked for:

Date: [DATE]  
Time: [TIME]  
Address: [ADDRESS]  
Package: [PACKAGE]  
Balance due: [$___]

Please make sure gifts are clearly labeled if Santa will hand them out.

## 48-Hour Reminder

Hi [NAME], confirming Papa Noel for [DATE] at [TIME].

Please confirm:

- Address
- Parking instructions
- Contact person
- Gift details
- Balance due

We are excited to be part of your event.

## Review Request

Thank you again for having Papa Noel at your event. It was a joy to be part of it.

If you have a moment, a short review or tagged post would help our family business so much. Even one sentence helps other families feel comfortable booking.

If you are comfortable, you can also send one approved photo we may share on the website or Instagram. We only repost photos with permission.

Proof page: https://miamipapanoel.com/reviews

## Proof Page Reply

Absolutely. Here are real visit moments, community proof, and photos from Miami Papa Noel:

https://miamipapanoel.com/reviews

If you send the date, city, event type, and guest count, I can help recommend the best visit option.

## Partner Reply

Absolutely. Here is the partner page for photographers, party vendors, pet businesses, restaurants, cafes, and local family businesses:

https://miamipapanoel.com/partners

Send the date idea, city, type of business, expected audience, and the kind of Santa moment you want to create. We can reply with the best visit option and availability.

## Referral Request

If you know another family, school, business, daycare, church, HOA, or nonprofit planning a holiday event, we would be grateful for the referral.
